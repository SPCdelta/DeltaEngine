	...
	: Scene(sceneName)
{
	...
	menuImage->AddComponent<Sprite>("player");
	...
	//sheetedSprite->AddComponent<Sprite>("Assets\\Textures\\spritesheet", sheet);
	std::shared_ptr<AnimationSheet> sheet = std::make_shared<AnimationSheet>(sheetedSprite->GetComponent<Transform>(), 4, 64, 64, 0, 0, 0, 2);
	sheet->AddIdleAnimation(4, 1, 200);
	sheetedSprite->AddComponent<Sprite>("spritesheet3", sheet);
