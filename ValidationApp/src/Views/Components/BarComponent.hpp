...
{
public:
	BarComponent(Scene& scene, const std::string& fontName, const Math::Vector2& startPos, const Math::Vector2& scale, Player* player, const std::string& spriteName);
...
protected:
	...
	int _maxValue;
	int _value;
	float _unit; // Represents the amount of units for each hitpoint lost to reduce the healthbar width.
